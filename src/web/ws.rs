...
/// Implementation moved to ws_resume.rs (§10.1).
async fn deliver_resume_if_pending(
    state: &AppState,
    sender: &mut futures_util::stream::SplitSink<WebSocket, WsMessage>,
) {
    crate::web::ws_resume::deliver_resume_if_pending(state, sender).await;
}
...
async fn handle_plan_decision(
    ...
    let plan = match crate::web::ws_plans::load_pending_plan(&state.config.general.data_dir, session_id) {
        Some(p) => p,
        None => {
    ...
    if approved {
        ...
        crate::web::ws_plans::delete_pending_plan(&state.config.general.data_dir, session_id);
